    ...
    public final void registerSubscriber(Subscriber subscriber, String... channels) {
        for(String channel : channels) {
            Thread thread = new Thread(() -> {
                try {
                    Chat.getLogger("&7[RAGC] &b" + plugin.getDescription().getName() + "&3 subscribing to Redis channel &b" + channel + "&3.");
                    rxRedis.subscribe(subscriber, channel);
                } catch (Exception e) {
                    if(Reference.logLevel >= 1) Chat.getLogger("&7[RAGC] &4" + plugin.getDescription().getName() + " caught an exception: " + e.getMessage() + "!", "severe");
